import java.util.ArrayList;

public class WebSort {
	private ArrayList<WebPage> webs;
	
	public WebSort(ArrayList<WebPage> webs){
		this.webs = webs;
    }
	
//	public void add(Keyword keyword){
//		lst.add(keyword);
//    }
	
	//Quick sort
	public void sort(){
		if(webs.size() == 0)
		{
			System.out.println("InvalidOperation");
		}
		else 
		{
			quickSort(0, webs.size()-1);
		}
	}
	
	private void quickSort(int leftbound, int rightbound){
		//1. Implement QuickSort algorithm
		if(leftbound >= rightbound) {
			return;
		}
		
//		if(leftbound < rightbound) {
//			int i = leftbound - 1;
//			for(int j = leftbound; j <= rightbound -1; j++) {
//				
//				if(webs.get(i).score < webs.get(rightbound).score) {
//					i++;
//					swap(i, j);
//				}
//			}
//			
//			swap((i+1), rightbound);
//			
//			quickSort(leftbound, i);
//			quickSort(i+2, rightbound);
//		}
		
		int p = rightbound;
		int j = leftbound;
		int k = rightbound - 1;
		
		while(j<k){
			while(webs.get(j).score < webs.get(p).score) {
				j++;
			}
			while(webs.get(k).score >= webs.get(p).score) {
				k--;
			}
			if(j<k) {
				swap(j,k);
			}
//			if(lst.get(j).count < lst.get(p).count) {
//				j++;
//				if(lst.get(k).count > lst.get(p).count) {
//					k--;
//				}
//			}
//			swap(j,k);
			
//			if(lst.get(k).count > lst.get(p).count) {
//				k--;
//			}
//			if(lst.get(j).count > lst.get(p).count && lst.get(k).count < lst.get(p).count) {
//				System.out.println(j);
//				System.out.println(k);
//				swap(j,k);
//			}
		}
		swap(j,p);
		quickSort(leftbound, j-1);
		quickSort(j+1, rightbound);
	}
	
	private void swap(int aIndex, int bIndex){
		WebPage temp = webs.get(aIndex);
		webs.set(aIndex, webs.get(bIndex));
		webs.set(bIndex, temp);
	}
	
	public void output(){
		StringBuilder sb = new StringBuilder();
		
		for(int i = 0; i < webs.size(); i++){
			String s = webs.get(i).name + " " + webs.get(i).score + "\n";
			if(i > 0)sb.append(" ");
			sb.append(s);
		}
		
		System.out.println(sb.toString());	
	}
}